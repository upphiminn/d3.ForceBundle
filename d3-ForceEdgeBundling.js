...
(function () {
	d3.ForceEdgeBundling = function () {
		...
		function edge_length(e) {
			...
			if (
				Math.abs(data_nodes[e.source].x - data_nodes[e.target].x) <
					eps &&
			...
			for (var i = 0; i < data_edges.length; i++) {
				if (P_initial === 1) {
					...
					subdivision_points_for_edge[i].push(
						data_nodes[data_edges[i].source]
					);
					subdivision_points_for_edge[i].push(
						data_nodes[data_edges[i].target]
					);
				}
			}
		}
		...
		function filter_self_loops(edgelist) {
			...
			for (var e = 0; e < edgelist.length; e++) {
				if (
					...
					data_nodes[edgelist[e].source].y !=
						data_nodes[edgelist[e].target].y
				) {
					//or smaller than eps
		...
		function apply_electrostatic_force(e_idx, i) {
			...
			for (var oe = 0; oe < compatible_edges_list.length; oe++) {
				var force = {
					x:
						subdivision_points_for_edge[compatible_edges_list[oe]][
							i
						].x - subdivision_points_for_edge[e_idx][i].x,
					y:
						subdivision_points_for_edge[compatible_edges_list[oe]][
							i
						].y - subdivision_points_for_edge[e_idx][i].y,
				};
				...
				if (Math.abs(force.x) > eps || Math.abs(force.y) > eps) {
					var diff =
						...
						Math.pow(
							custom_edge_length({
								source: subdivision_points_for_edge[
									compatible_edges_list[oe]
								][i],
		...
		function apply_resulting_forces_on_subdivision_points(e_idx, P, S) {
			...
			for (var i = 1; i < P + 1; i++) {
				...
				resulting_force.x =
					S * (spring_force.x + electrostatic_force.x);
				resulting_force.y =
		...
		function edge_visibility(P, Q) {
			...
			return Math.max(
				0,
				1 -
					(2 * euclidean_distance(midP, midI)) /
						euclidean_distance(I0, I1)
			);
		}
			...
			for (var cycle = 0; cycle < C; cycle++) {
				for (var iteration = 0; iteration < I; iteration++) {
					...
					for (var edge = 0; edge < data_edges.length; edge++) {
						forces[edge] =
							apply_resulting_forces_on_subdivision_points(
								edge,
								P,
								S
							);
					...
					for (var e = 0; e < data_edges.length; e++) {
						for (var i = 0; i < P + 1; i++) {
							subdivision_points_for_edge[e][i].x +=
								forces[e][i].x;
							subdivision_points_for_edge[e][i].y +=
								forces[e][i].y;
						}
					}
				}
